function [xi, fxi] = getCrossings(x1,fx1,x2,fx2)
% pos = getCrossings(x1,fx1,x2,fx2)
%
% getCrossings approximates the places where two functions cross. pos(1,:) 
% and pos(2,:) are the x and y coordinates of those crossing points.

[xi fxi] = intersections(x1,fx1,x2,fx2);
end
...
if sum(size(x1) > 1) ~= 1 || sum(size(y1) > 1) ~= 1 || length(x1) ~= length(y1)
	error('X1 and Y1 must be equal-length vectors of at least 2 points.')
end
% x2 and y2 must be vectors with same number of points (at least 2).
if sum(size(x2) > 1) ~= 1 || sum(size(y2) > 1) ~= 1 || length(x2) ~= length(y2)
